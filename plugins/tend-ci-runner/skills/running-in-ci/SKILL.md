...
- **Secrets**: Never run commands that introspect the process env (`env`, `printenv`, `set`, `export`) or `cat`/`echo` credential files. The rule is absolute — name-stripping filters like `env | cut -d= -f1` do not make these commands safe: the harness may place credential-bearing values in the environment (the Codex harness passes the PAT and model auth directly to the agent), and a single unfiltered `env` or `printenv FOO` prints the value verbatim into the session log, which is uploaded as an artifact. It covers reading a *process* as well as the shell's own env — `ps aux`, `ps -ef`, `pgrep -a`, `cat /proc/<pid>/cmdline`, `cat /proc/<pid>/environ`. The sandbox agent is launched as `env K=V K=V … bash …`, so its entire environment sits in that process's argv, and an innocuous "is the build still running?" listing dumps the lot into the log without ever looking like a secrets probe. Narrow instead of skipping the check: `pgrep` without `-a`, or `ps` restricted to non-argv columns (`ps -o pid,etime,comm`). Never include tokens or credentials in responses or comments.
