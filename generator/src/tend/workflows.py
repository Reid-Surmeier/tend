...
def generate_install_test(cfg: Config) -> GeneratedWorkflow:
    ...
    generator = (
        "uvx --from "
        + shlex.quote(
            f"git+https://github.com/{cfg.action_source}#subdirectory=generator"
        )
        + " tend"
        if cfg.action_source
        else 'uvx "tend@$TEND_VERSION"'
    )
    content = f"""\
{HEADER}
    ...
    steps:
      ...
      - name: Verify generator output matches committed files
        ...
        run: |
          ...
          {generator} init --with-install-test
...
def generate_all(
    ...
    for name, gen_fn in GENERATORS.items():
        ...
        results.append(wf)
    lifecycle = cfg.workflows.get("lifecycle", WorkflowConfig(enabled=False))
    if lifecycle.enabled:
        results.append(_apply_extras(_generate_scheduled(cfg, "lifecycle"), lifecycle))
    if with_install_test:
        ...
        results.append(wf)
    if cfg.action_source:
        command = f"uvx --from 'git+https://github.com/{cfg.action_source}#subdirectory=generator' tend init"
        for wf in results:
            wf.content = wf.content.replace("uvx tend@latest init", command, 1)
    return results
